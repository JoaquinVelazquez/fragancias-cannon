...
<div class="table-responsive">
    <div class="table-wrapper">
      <div class="table-title">
        <div class="row">
          ...
          @if(count($historias) < 1)
            <a href="/crearhistoria" class="btn btn-primary"><i class="fas fa-plus-circle"></i> <strong>Crear</strong></a>      
          @else
      ...
      @foreach($historias as $historia)
        <p>{{$historia->texto}}</p>
        <form action="historia/{{$historia->id}}/delete" method="POST">
        @csrf
        @method('DELETE')
        <a href="/historia/{{$historia->id}}/editarhistoria" class="btn btn-success enlace">Editar</a>
        @if(count($historias) > 1)
