...
<div class="table-responsive">
    <div class="table-wrapper">
      <div class="table-title">
        <div class="row">
          <div class="col-sm-6">
            <h2><b>Agregar Presentación por Variedad</b></h2>
          </div>
        </div>
      </div>
      ...
      <form action="/presentacionesporvariedades" method="POST">
        @csrf
        ...
        <div class="mb-3 d-flex" style="flex-direction: column;">
          ...
          <select name="presentacion_id" id="presentacion_id" class="form-control">
            ...
            <option value="{{$presentacion->id}}" {{ old('presentacion_id') == $presentacion->id ? 'selected' : ''}}>{{$presentacion->descripcion}}</option>
            @endforeach
          </select>
        </div>

        <div class="mb-3 d-flex" style="flex-direction: column;">
          <label for="variedad_id" class="form-label">Variedad:</label>
          <select name="variedad_id" id="variedad_id" class="form-control">
            <option value="" selected disabled>Seleccione una variedad</option>
            @foreach($variedades as $variedad)
            <option value="{{$variedad->id}}" {{ old('variedad_id') == $variedad->id ? 'selected' : ''}}>{{$variedad->id}}</option>
            @endforeach
          </select>
        </div>

        <div class="mb-3 d-flex" style="flex-direction: column;">
          <label for="vap" class="form-label">Vaporizador:</label>
          <select name="vap" id="vap" class="form-control">
            <option value="" selected disabled>Seleccione una opción</option>
            <option value="1" {{old('vap') === '1' ? 'selected' : ''}}>Si</option>
            <option value="0" {{old('vap') === '0' ? 'selected' : ''}}>No</option>
          </select>
        </div>
        ...
        <div class="mb-3 d-flex" style="flex-direction: column;">
          ...
          <select name="medida" id="medida" class="form-control">
            <option value="" selected disabled>Seleccione una opción</option>
            <option value="ml" {{old('medida') == 'ml' ? 'selected' : ''}}>ML</option>
            <option value="gs" {{old('medida') == 'gs' ? 'selected' : ''}}>GS</option>
            <option value="0" {{old('medida') === '0' ? 'selected' : ''}}>Ninguna</option>
          </select>
        </div>

        <div class="mb-3 d-flex" style="flex-direction: column;">
          <label for="vol" class="form-label">Volumen:</label>
          <input type="number" class="form-control" id="vol" name="vol" value="{{old('vol')}}">
        </div>

        <button type="submit" class="btn btn-primary">Agregar</button>
        <a href="/presentacionesporvariedades" class="btn btn-danger">Cancelar</a>
      </form>
