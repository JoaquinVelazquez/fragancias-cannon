...
<div class="table-responsive">
    <div class="table-wrapper">
      ...
      <form action="/perfumes" method="POST">
        @csrf
        <div class="mb-3">
          <label for="nombre" class="form-label">Nombre:</label>
          <input require type="text" class="form-control" id="nombre" name="nombre" value="{{old('nombre')}}">
        </div>

        <div class="mb-3">
          <label for="fragancia" class="form-label">Fragancia:</label>
          <input require type="text" class="form-control" id="fragancia" name="fragancia" value="{{old('fragancia')}}">
        </div>
        ...
        <div class="mb-3">
          ...
          @foreach($lineas as $linea)
            <option value="{{$linea->id}}" {{ old('linea_id') == $linea->id ? 'selected' : ''}}>{{$linea['nombre']}}</option>
          @endforeach
          </select>
        </div>
