...
<div class="table-responsive">
    <div class="table-wrapper">
      ...
      <form action="/variedades/{{$variedad->id}}/update" method="POST">
        @csrf
        @method('PUT')
        <div class="mb-3 d-flex" style="flex-direction: column;">
          <label for="nombre" class="form-label">Perfume:</label>
            <select name="perfume_id" id="perfume_id" class="form-control">
                <option value="" selected disabled>Seleccione un perfume</option>
            @foreach($perfumes as $perfume)
                <option value="{{$perfume->id}}" @if($perfume->id == $variedad->perfume->id) selected @endif>{{$perfume->nombre}}</option>
            @endforeach
            </select>
        </div>

        <div class="mb-3 d-flex" style="flex-direction: column;">
          <label for="codigo_catalogo" class="form-label">Codigo de catalogo:</label>
            <input type="text" class="form-control" id="codigo_catalogo" name="codigo_catalogo" value="{{$variedad->codigo_catalogo}}">
        </div>

        <div class="mb-3 d-flex" style="flex-direction: column;">
          <label for="codigo_ean" class="form-label">Codigo EAN:</label>
            <input type="text" class="form-control" id="codigo_ean" name="codigo_ean" value="{{$variedad->codigo_ean}}">
        </div>

        <div class="mb-3 d-flex" style="flex-direction: column;">
          <label for="precio" class="form-label">Precio:</label>
            <input type="number" class="form-control" id="precio" name="precio" min="0" value="{{$variedad->precio}}">
        </div>
        ...
        <div class="mb-3 d-flex" style="flex-direction: column;">
          <label for="activo" class="form-label">Activo:</label>
            ...
            <option value="1" @if($variedad->activo) selected @endif>Si</option>
            <option value="0" @if(!($variedad->activo)) selected @endif>No</option>
            </select>
        </div>

        <div class="mb-3 d-flex" style="flex-direction: column;">
          <label for="activo" class="form-label">Limitado:</label>
            <select name="limited" id="limited" class="form-control">
            <option value="" disabled selected>Seleccione una opción</option>
            <option value="1" @if($variedad->limited) selected @endif>Si</option>
            <option value="0" @if(!($variedad->limited)) selected @endif>No</option>
            </select>
        </div>

        <button type="submit" class="btn btn-primary">Actualizar</button>
        <a href="/variedades" class="btn btn-danger">Cancelar</a>
      </form>
